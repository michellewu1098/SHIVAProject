#ifndef SDF_LOOKUP_BVH_ACCELERATED_INCLUDED
#define SDF_LOOKUP_BVH_ACCELERATED_INCLUDED
...
{
	//----------------------------------------------------------------------------------------------------------------------
	/// @class bvh_accelerated "include/sdf/lookup/bvh_accelerated.hpp"
	/// @brief Using a bvh it will look for the shortest distance to the mesh
	/// @author Mathieu Sanchez
	/// @version 1.0
	/// @date Last Revision 28/06/11 Initial revision
	//----------------------------------------------------------------------------------------------------------------------
	class bvh_accelerated
	{
	public :
	...
	public :
		...
		void operator()(const point3d i_positions[], distance_record o_closest_record[], unsigned int o_face_id[]) const;

		const detail::bvh& bvh() const { return m_tree; }
		std::size_t memory_usage() const { return m_tree.memory_usage(); }
	private :
		//----------------------------------------------------------------------------------------------------------------------
		...
		const mesh* m_mesh;
		//----------------------------------------------------------------------------------------------------------------------
		/// @brief A BVH tree to accelerate lookups
		//----------------------------------------------------------------------------------------------------------------------
		detail::bvh m_tree;
	};
}

#endif /* SDF_LOOKUP_BVH_ACCELERATED_INCLUDED */
