...
{
    float t = time;
    float va = texture( volume1, texcoords ).a;
    float vb = texture( volume2, texcoords ).a;
...
void main ()
{
        ...
        vec3 dir = normalize( o_WorldSpacePos - cam );
        ...
        for( i = 0; i < nIterations; i++ )
        {
                ...
                if( sample * multiplier > isoValue * multiplier )
                {
                        ...
                        // We found a solution, stop the loop and proceed to shading
                        break;
                }
        ...
        vec3 diffColour = fieldColour( pos, -texture( volume1, texCoord ).r, -texture( volume2, texCoord ).r );
        // proceed to shading
		...
		fragColour = vec4( clamp( ( diffColour * LdotN ) * 0.8 + 0.2 * diffColour, 0.0, 1.0 ), 1 );
}
