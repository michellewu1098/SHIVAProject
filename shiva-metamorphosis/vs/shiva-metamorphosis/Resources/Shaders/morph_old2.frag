...
uniform float a0 = 12.0;
uniform float a1 = 5.0;
uniform float a2 = 5.0;
uniform float a3 = 0.1;
uniform float alpha;
uniform float clipdist = 0.05;
uniform float boundplanedist = 10;
...
float getField(vec3 texcoords)
{
    ...
    float fin = f1 + f2 + sqrt(f1*f1+f2*f2) + d;
...
void main ()
{
        ...
        float stepSize = parameters.z*0.1;
